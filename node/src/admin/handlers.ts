...
/**
 * Holds config, audit log, and the **persistent** Mailcow client.
 *
 * Since v0.4.0-alpha the client is reused across all tool calls (was
 * per-call). This avoids TLS handshake overhead and prevents the dispatcher
 * from being left in a broken state after a transient error. Call
 * `aclose()` at server shutdown.
 */
...
export function makeContext(config: AdminConfig, audit: AuditLogger): AdminContext {
  const client = new MailcowClient({
    ...
    timeoutMs: config.apiTimeoutMs ?? 60_000,
  });
  return {
    config,
    audit,
    client,
    aclose: async () => {
      // undici Agent has a close() method. Best-effort.
      try {
        await client.aclose();
      } catch {
